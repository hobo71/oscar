<template>
  <div class="modal fade" tabindex="-1" role="dialog" id="alquileresEditModal">
          ...
          <div class="card-footer bg-warning text-dark rounded-0 border-dotted-top">
            <div class="d-flex justify-content-between align-items-center">
              ...
              <div class="text-right">
                ...
                <h1 class="m-0">
                  <strong>${{cotizacionComputed}}</strong>
                </h1>
              </div>
            </div>
          </div>
...
export default {
  ...
  mounted() {
    this.alquiler = this.alquilerData
    axios
      .all([
        this.getClientes(),
        this.getVehiculos(),
  ...
  methods: {
    update() {
      this.alquiler.cotizacion =  this.cotizacionComputed;
      console.log("alquiler before update-> ", this.alquiler);
      axios
        .put(this._appApi+"alquileres/" + this.alquiler.id, this.alquiler)
        .then(response => {
          console.log(response.data);
          this.$root.$emit("alquileresUpdate");
          $("#alquileresEditModal").modal("hide");
        })
        .catch(e => console.log(e));
    },
    getClientes() {
