<template>
  <div class="modal fade" tabindex="-1" role="dialog" id="clienteEditModal">
...
export default {
  ...
  mounted() {
    ...
    this.getEmpresas();
    // setInterval( ()=> {
    //   this.cliente = this.$parent.cliente
  ...
  methods: {
    update() {
      axios
        .put(this._appApi+"clientes/"+this.cliente.id, this.cliente)
          .then(response => {
            console.log(response.data);
            this.$root.$emit("clienteUpdate");
